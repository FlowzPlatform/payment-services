  ...
  console.log(_.isEmpty(myObj));
   return new Promise((resolve, reject) => {
     ...
     if(!_.has(myObj, 'id') && _.has(myObj, 'gateway'))  {
       ...
       let gatewayUsed = eval(hook.params.query.gateway + "_subscription_get_schema");
...
before_create_subscription = (hook) => {
  let gatewayUsed = eval(hook.data.gateway + "_subscription_create_schema");
  let validate = ajv.compile(gatewayUsed);
...
function before_delete_subscription (hook) {
  //console.log(hook);
  return new Promise((resolve, reject) => {
    let gatewayUsed = eval(hook.params.query.gateway + "_subscription_delete_schema");
    let validate = ajv.compile(gatewayUsed);
    let valid = validate(hook.params.query);
    if (!valid)
    {
